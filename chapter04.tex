...
*** Средства разработки
C++, Boost library, argparse, tinyxml2
...
Корневой XML-элемент (тег) \textit{annotation} имеет опциональный атрибут \textit{pipeline}, предназначенный для построения высокоуровневых команд обработки исходных текстов.
...
Кроме атрибута \textit{pipeline} корневой элемент содержит в себе следующие основные элементы:
...
В листинге~\ref{annotation-pois-example} приведен пример фрагмента аннотации, в котором описываются несколько ``точек интереса'', такие как \textit{class\_name}, \textit{method\_name} и \textit{if\_condition}.
...
В рассмартриваемом примере точка интереса с именем \textit{class\_name} преобразовывается в TXL функцию, исходный текст которой представлен в листинге~\ref{poi-src-example}.
...
Из приведенного листинга видно, что это фрагмент с именем \textit{logging\_message} предназначается для использования при инструментировании программного кода на языке Java (атрибут $language$).
При этом фрагмент принуждает поьзователя использовать данный фрагмент с некоторыми другими фрагментами с именами \textit{logging\_imports} и \textit{logging\_fields} соответственно.
